...
from transformers.models.mt5.modeling_mt5 import (
    MT5Stack, MT5Block, MT5LayerNorm, MT5LayerSelfAttention, MT5LayerFF, MT5LayerCrossAttention,
    MT5PreTrainedModel, MT5ForConditionalGeneration
)
import torch
...
class MERGE(MT5ForConditionalGeneration):
    ...
    def ranking_loss(self, lm_logits, labels):
        weights = torch.tensor([1.0/math.sqrt(i+1) for i in range(labels.shape[1])]).to('cuda')
